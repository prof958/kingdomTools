...
- [x] Turn Tracker (roll, then confirm). `src/lib/pf2e/kingdom-turn.ts` (17 tests): pure
      Upkeep-phase math — Adjust Unrest delta, Commodity gains per Work Site (doubled on
      a Resource-feature hex), storage-capped stockpiling, Consumption (settlement total
      − Farmland hexes), the 5-RP-per-point Consumption shortfall cost. All dice are
      rolled client-side (`lib/dice.ts`) and land in an editable field before anything is
      written — that's the actual "roll, then confirm" mechanism, not a separate
      approval step. `GET/POST /api/kingdom/turns` (resume the in-progress turn rather
      than duplicating it) + `PATCH /api/kingdom/turns/[id]` (log a step / mark done /
      complete — completing bumps `kingdom.currentTurn` in the same transaction).
      Numeric consequences go through the *existing* `PATCH /api/kingdom` — this route
      only owns the turn's own bookkeeping, so there's exactly one place that writes
      kingdom stats.
      The 4 Leadership/Region/Civic/Army activity steps and Commerce's Collect Taxes all
      share one `ActivityCheckStep` component: pick an activity from the 51-entry
      catalog (Civic excludes Build Structure — that stays on the Settlements tab), pick
      a skill, roll against Control DC using the kingdom's real ability scores/ranks/
      invested bonuses/Ruin penalties, and the matching degree-of-success outcome text
      from the catalog displays automatically. This is where the activity catalog, dice
      module, and skill-modifier math finally meet.
      Added `unrestStatusPenalty()` — the Unrest 1/5/10/15 status-penalty tiers (KPG 39)
      were never modeled anywhere; the turn tracker's rolls are the first thing in the
      app to apply it. The Skills tab's displayed modifiers still don't include it —
      known gap, noted rather than silently inconsistent.
      `KINGDOM_TURN_PHASES`' two XP-era Event steps (`apply-xp`, `increase-level`)
      replaced with one `milestone-check` step, matching the milestone-levelling
      decision — they referenced kingdom XP tracking, which the memory bank already
      says is a house-ruled decision.
      Structure cost, and every activity's outcome text, are shown but not
      auto-applied — same "player-helper, not rules-police" precedent as Settlements'
      Build Structure: the text is often prose ("gain RP equal to the result of 2
      Resource Dice") that isn't safe to parse and auto-apply.
      Verified live end-to-end against the real dev DB: started Turn 1, rolled Resource
      Collection (5d4 → 11, confirmed `kingdoms.rp = 11` in Postgres), rolled a Capital
      Investment check (17 vs DC 14 → Success, logged with the exact catalog outcome
      text), completed the turn (`kingdoms.current_turn` advanced to 1, the turn's own
      row flipped to `complete` with a summary) — then confirmed "Start Turn 2" was
      offered next, proving the whole lifecycle round-trips correctly.
